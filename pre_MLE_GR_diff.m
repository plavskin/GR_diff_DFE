...
%    pp_parameters_in_list = intersect(parameter_list, petite_prop_mle_parameters);
    ...
    % Calculate how often each strain from strain_list occurs in
        % test_strain_list_by_pair
    test_strain_occurances = ...
        cell2mat(cellfun(@(x) sum(ismember(test_strain_list_by_pair, x)), ...
            strain_list_from_me_param_list, 'un',0));
    ...
    output_dict = containers.Map({'strain_list', 'test_strain_list_by_pair', 'GR_diff_list', ...
        'test_strain_ML_file', 'strain_LL_table_file', 'test_strain_occurances'}, ...
