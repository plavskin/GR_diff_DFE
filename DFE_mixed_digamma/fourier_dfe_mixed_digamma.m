function [Fa, Fa_gradient_dict, complete_me_parameter_list, ...
    ...
    complete_me_parameter_list = ...
        ...
        mle_parameter_names(contains(mle_parameter_names, '_unseq_single_muts'))];
    ...
    fitted_parameters_unseq_single_muts_fullname = ...
        fitted_parameters(contains(fitted_parameters, '_unseq_single_muts'));
    fitted_parameters_unseq_single_muts = ...
        strrep(fitted_parameters_unseq_single_muts_fullname, '_unseq_single_muts', '');
    ...
    % Calculate Fz_unseq_single_muts, the fourier transform of the pdf of observing a
        % single mutational effect from a digamma distribution with unsequenced
        % mutation parameters
    % Then calculate Fa_unseq_single_muts, the fourier transform of the pdf of observing
        % a poisson random number of mutational effects from the
        % digamma distribution in Fz_unseq_single_muts
    [Fz_unseq_single_muts, Fz_unseq_single_muts_gradient_dict_temp_names] = ...
        fourier_domain_digamma(me_pdf_frequency_vals, mu_unseq_single_muts, ...
            shape_unseq_single_muts, prop_pos_unseq_single_muts, ...
        fitted_parameters_unseq_single_muts, gradient_specification);
    [Fa_unseq_single_muts, Fa_unseq_single_muts_gradient_dict_temp_names] = ...
        fourier_domain_poisson_dist_num(Fz_unseq_single_muts, ...
            me_pdf_frequency_vals, lambda_unseq_single_muts, ...
            Fz_unseq_single_muts_gradient_dict_temp_names, ...
            fitted_parameters_unseq_single_muts, gradient_specification);
    ...
    if gradient_specification
        ...
        corrected_unseq_single_muts_gradient_dict_keys =  ...
            strcat(keys(Fa_unseq_single_muts_gradient_dict_temp_names), '_unseq_single_muts');
        Fa_unseq_single_muts_gradient_dict = ...
            containers.Map(corrected_unseq_single_muts_gradient_dict_keys, ...
                values(Fa_unseq_single_muts_gradient_dict_temp_names));
    else
        Fa_SNM_gradient_dict = containers.Map('KeyType', 'char', ...
            'ValueType', 'any');
        Fa_unseq_single_muts_gradient_dict = containers.Map('KeyType', 'char', ...
            'ValueType', 'any');
    end

    % Create fourier transform of distribution of mutational effects in
        % a strain containing both SNMs and unsequenced mutations
    [Fa, Fa_gradient_dict] = derivative_multiplier(Fa_SNM, Fa_unseq_single_muts, ...
        Fa_SNM_gradient_dict, Fa_unseq_single_muts_gradient_dict);
